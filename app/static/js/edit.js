document.getElementById('image').addEventListener('change', function(event) {
    ...
    if (file) {
        ...
        reader.onload = function(e) {
            ...
            img.style.maxHeight = '200px'; // Optional: limit the image size
...
function validateImage() {
    ...
    // Check if an existing image is already displayed (e.g., from the database)
    const existingImage = document.getElementById('existingImage');

    // If there's no existing image and no new image is uploaded, validation should fail
    ...
    if (imageInput.files.length > 0) {
        ...
        if (!allowedTypes.includes(file.type)) {
            errorContainer.textContent = '- Please upload a valid image file (JPG or PNG).';
            return false;
        }
    }

    // If either an existing image or a valid new image exists, clear the error message
...
// Handle form submission only for the Edit form
